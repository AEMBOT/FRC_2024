...
public class ShootingCommands {
    ...
    public static Command shootAmp(
        ...
        double targetPivotAngle = Units.degreesToRadians(100);
        double shooterAmpRPM = 1000;
        return Commands.run(
                () -> {
                        shooter.setVelocityRPMCommand(shooterAmpRPM);
                        pivot.setPositionCommand(() -> targetPivotAngle);
                }
        );
    }

    public static Command shootTrapFromGround(
        Drive drive,
        Pivot pivot, 
        Shooter shooter
    ){
        double targetPivotAngle = Units.degreesToRadians(110);
        double shooterTrapGroundRPM = 2000;
        return Commands.run(
                ()-> {
                        shooter.setVelocityRPMCommand(shooterTrapGroundRPM);
                        pivot.setPositionCommand(() -> targetPivotAngle);
                }
        );
    }

    public static Command shootTrapFromClimb(
        Drive drive, 
        Pivot pivot, 
        Shooter shooter
    ){
        double targetPivotAngle = Units.degreesToRadians(110);
        double shooterTrapClimbRPM = 2000;
        return Commands.run(
                () -> {
                        shooter.setVelocityRPMCommand(shooterTrapClimbRPM);
                        pivot.setPositionCommand(() -> targetPivotAngle);
                }
        );
    }
