...
public class RotateDriveToNote {
    ...
    //need to build in logic to take left, right, center of closest (y value) and/or xpixel based on closest
    //first with left, center, right
    //sequential command for rotate and drive
    //build in logic for y values which to do first, track the one with the highest y value at all times, when driving towards it, 
    ...
    public static Command DriveToNote(
        Drive drive, String notePosChar){
            return Commands.run(
                () -> {
                    double speed = 0;
